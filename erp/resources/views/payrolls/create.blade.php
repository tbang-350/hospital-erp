...
<div class="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="flex items-center justify-between mb-8">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    ...
                    <p class="text-gray-600 dark:text-gray-300 mt-1">Generate a new payroll record for an employee</p>
                </div>
            </div>
        </div>
        ...
        <div class="max-w-4xl mx-auto">
            <form action="{{ route('payrolls.store') }}" method="POST" class="space-y-8">
                @csrf

                <!-- Employee Selection -->
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Employee Selection</h3>
                        <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Choose the employee for this payroll</p>
                    </div>
                    <div class="p-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <select id="employee_id" name="employee_id" required onchange="loadEmployeeData(this.value)"
                                        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-gray-700 dark:text-white @error('employee_id') border-red-500 @enderror">
                                    ...
                                    @foreach($employees as $employee)
                                        <option value="{{ $employee->id }}" 
                                            ...
                                            {{ $employee->full_name }} - {{ $employee->position ?: 'No position' }} ({{ $employee->department ?: 'No department' }})
                                        </option>
                                    @endforeach
                                </select>
                                @error('employee_id')
                            ...
                            <div id="employee-info" class="md:col-span-2 hidden bg-blue-50 dark:bg-blue-900 rounded-lg p-4">
                                <h4 class="text-sm font-medium text-blue-900 dark:text-blue-100 mb-2">Employee Information</h4>
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                                    <div>
                                        <span class="text-blue-700 dark:text-blue-300 font-medium">Name:</span>
                                        <span id="emp-name" class="text-blue-900 dark:text-blue-100 ml-1"></span>
                                    </div>
                                    <div>
                                        <span class="text-blue-700 dark:text-blue-300 font-medium">Position:</span>
                                        <span id="emp-position" class="text-blue-900 dark:text-blue-100 ml-1"></span>
                                    </div>
                                    <div>
                                        <span class="text-blue-700 dark:text-blue-300 font-medium">Base Salary:</span>
                                        <span id="emp-salary" class="text-blue-900 dark:text-blue-100 ml-1"></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" id="period" name="period" value="{{ old('period', date('Y-m')) }}" required
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" id="basic_salary" name="basic_salary" value="{{ old('basic_salary') }}" required step="0.01" min="0" onchange="calculateNetPay()"
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" id="allowances" name="allowances" value="{{ old('allowances', 0) }}" step="0.01" min="0" onchange="calculateNetPay()"
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" id="deductions" name="deductions" value="{{ old('deductions', 0) }}" step="0.01" min="0" onchange="calculateNetPay()"
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input type="hidden" id="net_pay" name="net_pay" value="{{ old('net_pay', 0) }}">
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div>
                                ...
                                <textarea id="meta" name="meta" rows="4"
                                          ...
                                          placeholder="Additional notes, overtime hours, bonuses, etc. (JSON format or plain text)">{{ old('meta') }}</textarea>
                                @error('meta')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <div class="flex items-center space-x-4">
                                    <label class="flex items-center">
                                        <input type="radio" name="mark_as_paid" value="0" {{ old('mark_as_paid', '0') == '0' ? 'checked' : '' }}
                                               class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 dark:border-gray-600">
                                        <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Pending Payment</span>
                                    </label>
                                    <label class="flex items-center">
                                        <input type="radio" name="mark_as_paid" value="1" {{ old('mark_as_paid') == '1' ? 'checked' : '' }}
                                               class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 dark:border-gray-600">
                                        <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Mark as Paid</span>
                                    </label>
                                </div>
                                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Choose whether to mark this payroll as already paid</p>
                            </div>
                        </div>
                    </div>
                </div>
...
function loadEmployeeData(employeeId) {
    ...
    if (option && employeeId) {
        // Show employee info
        document.getElementById('emp-name').textContent = option.dataset.name || 'N/A';
        document.getElementById('emp-position').textContent = option.dataset.position || 'No position';
        document.getElementById('emp-salary').textContent = '$' + parseFloat(option.dataset.salary || 0).toLocaleString('en-US', {minimumFractionDigits: 2});
        
        // Set basic salary
        basicSalaryInput.value = option.dataset.salary || 0;
