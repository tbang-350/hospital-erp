...
<div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
    <form action="{{ route('inventory.update', $item) }}" method="POST" class="p-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" id="sku" value="{{ $item->sku }}" readonly 
            ...
            <div>
                ...
                <input type="number" id="current_quantity" value="{{ $item->quantity }}" readonly 
                       class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700 text-gray-500 dark:text-gray-400">
            ...
            <div>
                ...
                <select id="uom" name="uom" required class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:text-white">
                    ...
                    <option value="unit" {{ old('uom', $item->uom) == 'unit' ? 'selected' : '' }}>Unit</option>
                    <option value="box" {{ old('uom', $item->uom) == 'box' ? 'selected' : '' }}>Box</option>
                    <option value="bottle" {{ old('uom', $item->uom) == 'bottle' ? 'selected' : '' }}>Bottle</option>
                    <option value="vial" {{ old('uom', $item->uom) == 'vial' ? 'selected' : '' }}>Vial</option>
                    <option value="tablet" {{ old('uom', $item->uom) == 'tablet' ? 'selected' : '' }}>Tablet</option>
                    <option value="capsule" {{ old('uom', $item->uom) == 'capsule' ? 'selected' : '' }}>Capsule</option>
                    <option value="ml" {{ old('uom', $item->uom) == 'ml' ? 'selected' : '' }}>ML</option>
                    ...
                    <option value="pack" {{ old('uom', $item->uom) == 'pack' ? 'selected' : '' }}>Pack</option>
                </select>
                @error('uom')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" id="unit_cost" name="unit_cost" value="{{ old('unit_cost', $item->unit_cost) }}" min="0" step="0.01" required 
                       class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:text-white" 
                       placeholder="0.00">
                @error('unit_cost')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
            ...
            <div class="md:col-span-2">
                ...
                <input type="date" id="expiry_date" name="expiry_date" value="{{ old('expiry_date', $item->expiry_date?->format('Y-m-d')) }}" 
                       class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:text-white" 
...
<div class="mt-6 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
    <div class="flex items-center justify-between">
        ...
        <div class="ml-4">
            <a href="{{ route('inventory.stock-movement', $item) }}" class="inline-flex items-center px-3 py-2 bg-yellow-600 hover:bg-yellow-700 text-white text-sm font-medium rounded-lg transition-colors">
                Manage Stock
            </a>
        </div>
    </div>
